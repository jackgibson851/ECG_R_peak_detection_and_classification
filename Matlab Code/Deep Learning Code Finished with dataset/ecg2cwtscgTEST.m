function ecg2cwtscgTEST(ecgdata,cwtfb,ecgtype)
...
numsigs = 10;       %number of signals to be produced from each larger signal 
...
if ecgtype == 'ARR'    % if the ecgtype is ARR: go to the file in the created directory 
    pathToFolder = strcat('C:\Users\jackg\OneDrive\Documents\MATLAB\Digital_Signal_Processing\deep_learning\ecgTest\arr\');
    findx =0; 
    for i = 1:6     % do for all 6 signals 
        indx = 0;         
...
elseif ecgtype == 'CHF'  %if chf:  do the same but in chf directory  
    pathToFolder =strcat('C:\Users\jackg\OneDrive\Documents\MATLAB\Digital_Signal_Processing\deep_learning\ecgTest\chf\');
    findx = 0; 
    for i = 1:6
        indx=0;
        for k= 1:numsigs 
...
elseif ecgtype == 'NSR'   %if NSR:  do the same but in NSR directory
    pathToFolder =strcat('C:\Users\jackg\OneDrive\Documents\MATLAB\Digital_Signal_Processing\deep_learning\ecgTest\nsr\');
    findx = 0; 
    for i = 1:6
        indx=0;
        for k= 1:numsigs 
